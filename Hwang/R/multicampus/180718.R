...
library(MASS)
states <- as.data.frame(state.x77[,c("Muder","Population", "Illiteracy",
                                     "Income", "Frost")])
View(states)
cor(states)
...
mtcars
(m <- lm(formula = mpg ~ cyl + disp + wt + qsec, data = mtcars))
plot(m)
...
# 가장 적합한 모델
mm <- lm(formula = mpg ~ wt, data = mtcars)
plot(mm)
summary(mm)
avPlots(mm)
vif(mm)
