...
?subset # 조건을 만족하는 usbset을 반환한다.


View(subway)
# 역명 확인 : 158개 (오름차순정렬)
sort(unique(subway[ , 'stat_name']), decreasing = T)
sort(unique(subway[ , 'stat_name']), decreasing = F)
?sort
...
library(readxl)
subname <- read_excel("subway_latlong.xlsx")
?read_excel
str(subname)
...
# 세로에 year의 각 수준으로 나타내는 테이블을 맊들어 해당하는 on_tot값을 춗력함
...
df2 <- data.frame(stat_name=names(cum_total_vec), 
                  line_num=line_num, on_tot=cum_total_vec,
                  stringsAsFactors = FALSE) 
...
plt + theme_bw() + geom_bar(stat="identity", colour="white") + scale_x_discrete("지하첛역", labels=NULL) + ylab("탑승객수") + scale_fill_discrete(name=c("노선"))

# 총누적탑승객수를 기준으로 지하첛역을 순서화하여 상위 10개역을 추출
...
plt + geom_bar(stat="identity", colour="white") + xlab("상위 10개 지하첛역") + scale_y_continuous("탑승객수", lim=lim) + scale_fill_discrete(name=c("노선"))

# 총누적탑승객수 기준 하위 10개역을 추측하고 이를 막대그래프로 시각화
...
plt + geom_bar(stat="identity", colour="white") + xlab("하위 10개 지하첛역") + scale_y_continuous("탑승객수", lim=lim) + scale_fill_discrete(name=c("노선"))
...
plt <- plt + theme_classic() + geom_line() + geom_point(size=6, shape=19, alpha=0.5) 
plt + scale_x_discrete("2013년", labels=paste0(unique(as.numeric(month)), "월")) + ylab("월별 탑승객수") + scale_colour_discrete(name=c("지하첛역"))
...
pie(tmp2$on_tot, labels=paste0(unique(tmp2$LINE_NUM), "호선"), col=col, border="lightgray", main="노선별 평균 지하첛 탑승객 수")
barplot(tmp2$on_tot, labels=paste0(unique(tmp2$LINE_NUM), "호선"), col=col, border="lightgray", main="노선별 평균 탑승객 수")
...
formul <- admit~gpa+gre+rank
# mmm <- glm(formula = gre~gpa + admit + rank,data=mydata)
mmm <- glm(formula = formul,data=mydata)
mmm
mmmm <- glm( admit~gpa+rank,data=mydata)
glm(formul, family=binomial(link="logit"), data=mydata)
par(mfrow=c(2,2))
plot(mmm)
summary(mmm)
